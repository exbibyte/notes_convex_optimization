...
\begin{multicols*}{2}
 ...
 f convex $\iff \\ (\forall x,x_0 \in dom(f)) f(x) \geq f(x_0) + \nabla f(x_0)^T(x-x_0)$\\
  ...
  \begin{algorithm}[H]
    ...
    \While{$\frac{1}{2}\lambda(x)^2>\epsilon$}{
  ...
  Approaches:
  \begin{itemize}
  \item elimination of equality constraint, variable substitution into objective\\
    Solve remaining unconstrained problem.
  \item dual method: solve equality constrained primal using dual formulation
  \item modified Newton's Method with equality constraint handling
  \end{itemize}

  Solve via KKT condition at optimality:
  \begin{align*}
  ...
  Modified Newton's Method with equality contraint handling
