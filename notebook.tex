...
\begin{multicols*}{2}
 ...
 f convex $\iff \\ (\forall x,x_0 \in dom(f)) f(x) \geq f(x_0) + \nabla f(x_0)^T(x-x_0)$\\
  ...
  $f$ convex, but not $C^{1},C^{2}$\\
  Idea: need a $g$ such that $(\forall x)f(x) \geq f(x_0)+g^T(x-x_0)$\\
  $g:=$ a subgradient\\
  Can use $g$ in ellipsoid method\\
  ...
  Choosing $\lapha_k$, derivation ultimately comes to:\\
  ...
  \begin{algorithm}[H]
    \Do{until convergence}{
      solve QP to obtain $9\Delta x, \Delta \lambda, \Delta v)$\\
